...
class Summary:
    ...
    def add_writer(self, dir_sum, name):
        ...
        self._writers[name] = tf.compat.v1.summary.FileWriter(dir_sum, self._sess.graph, filename_suffix=name)

    def add_variable(self, name="name"):
        assert name not in self._vars, "Already has " + name
        var = tf.compat.v1.Variable(0.)
        tf.compat.v1.summary.scalar(name, var)
        self._vars[name] = var

    def build(self):
        self._ops = tf.compat.v1.summary.merge_all()
