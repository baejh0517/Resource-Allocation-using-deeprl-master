...
class DQNetwork:
    ...
    def __init__(self, session, dim_state, dim_action, learning_rate, tau=0.01):
        ...
        self._actions = tf.compat.v1.placeholder(tf.float32, [None, self._dim_a])
        self._y_values = tf.compat.v1.placeholder(tf.float32, [None])  #yahan change

        action_q_values = tf.compat.v1.reduce_sum(tf.multiply(self._out, self._actions), reduction_indices=1)  #yahan bih
        ...
        self.optimize = tf.compat.v1.train.AdamOptimizer(self._lr).minimize(self.loss)

    def buildNetwork(self, state, type):
        with tf.compat.v1.variable_scope(type):
        ...
        params = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope=type)
